-- ============================================================
-- E-Commerce Portfolio Database — Seed Data
-- Run AFTER schema.sql.
-- ============================================================
--
-- 20 customers, 15 products, 35 orders, 31 payments.
-- Designed so the analytical queries return non-trivial results:
--   * Multiple customers with multiple orders (LTV, RFM)
--   * Two-year span 2022–2023 (cohorts, YoY)
--   * 3 orders without payment (data quality demo)
--   * Mixed statuses (cancelled, refunded, completed)
-- ============================================================
...
-- ------------------------------------------------------------
-- ORDERS  (statuses: completed · cancelled · refunded)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- PAYMENTS
-- Note: not every order has a payment row.
--   * order #5  — status 'refunded'   (no payment expected)
--   * order #12 — status 'cancelled'  (no payment expected)
--   * order #7  — status 'completed'  ← data-quality anomaly
--   * order #34 — status 'completed'  ← data-quality anomaly
-- Query 8 surfaces #7 and #34: completed orders missing a payment
-- row. Status-based exclusions keep the refund/cancellation rows
-- out of the result.
-- ------------------------------------------------------------
